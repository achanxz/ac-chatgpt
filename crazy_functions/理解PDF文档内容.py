...
from toolbox import CatchException, report_execption  # 捕获和处理异常
...
def 解析PDF(file_name, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt):
    ...
    from .crazy_utils import breakdown_txt_to_satisfy_token_limit_for_pdf  # 文本字符串切割
    from request_llm.bridge_all import model_info  # chatgpt的类型
    ...
    for i in range(n_fragment):
        ...
        i_say = f"阅读这一部分，用不超过{NUM_OF_WORD}个文字理解这部分内容: {paper_fragments[i]}"
        i_say_show_user = f"[{i + 1}/{n_fragment}]阅读这一部分，用不超过{NUM_OF_WORD}个文字理解这部分内容: {paper_fragments[i][:200]}"
